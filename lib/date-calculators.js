...
function calendarBreakdown(start, end) {
  ...
  let years = 0
  while (addYearsClamped(cursor, 1) <= end && years < 500) { cursor = addYearsClamped(cursor, 1); years += 1 }
  let months = 0
  while (addMonthsClamped(cursor, 1) <= end && months < 12) { cursor = addMonthsClamped(cursor, 1); months += 1 }
...
export function calculateCountdown(targetInput, nowInput = Date.now()) {
  ...
  return { milliseconds, totalSeconds, days, hours, minutes, seconds }
}
...
export function calculateTimeDuration(startInput, endInput) {
  ...
  return { milliseconds, totalMinutes, totalHours: milliseconds / 3600000, days, hours, minutes, seconds }
}
