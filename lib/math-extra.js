...
export function solveQuadratic(aInput, bInput, cInput) {
  ...
  const vertexX = normalizeZero(-b / (2 * a))
  const vertexY = normalizeZero(a * vertexX * vertexX + b * vertexX + c)
  ...
  if (discriminant >= 0) {
    ...
    const root1 = normalizeZero((-b + sqrt) / (2 * a))
    const root2 = normalizeZero((-b - sqrt) / (2 * a))
    return { discriminant: normalizeZero(discriminant), kind: discriminant === 0 ? 'repeated' : 'real', roots: [root1, root2], vertexX, vertexY }
  }
  const real = normalizeZero(-b / (2 * a))
