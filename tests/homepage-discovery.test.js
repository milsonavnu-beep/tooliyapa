...
describe('homepage category and tool discovery',()=>{
  ...
  it('makes PDF Tools, Calculators, Converters, and Documents active while deriving counts from registries',()=>{expect(home).toContain('Browse by category');expect(home).toContain('href="#pdf-tools"');expect(home).toContain('{PDF_TOOLS.length} tools available');expect(home).toContain('href="/calculators"');expect(home).toContain('{AVAILABLE_CALCULATORS.length} tools available');expect(home).toContain('href="/converters"');expect(home).toContain('{AVAILABLE_CONVERTERS.length} tools available');expect(home).toContain('href="/documents"');expect(home).toContain('{AVAILABLE_DOCUMENT_TOOLS.length} tools available');expect(AVAILABLE_CALCULATORS.length).toBeGreaterThan(20);expect(AVAILABLE_CONVERTERS.length).toBeGreaterThan(15);expect(AVAILABLE_DOCUMENT_TOOLS).toHaveLength(14)})
  it('keeps only Text as a future category',()=>{expect(home).toContain("title:'Text'");expect(home).toContain('aria-disabled="true"');expect(home).toContain('Coming soon');expect(home).not.toMatch(/title:'Documents'[\s\S]*Coming soon/)})
