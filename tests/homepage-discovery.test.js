...
describe('homepage category and tool discovery',()=>{
  ...
  it('makes PDF Tools, Calculators, and Converters active while deriving counts from registries',()=>{expect(home).toContain('Browse by category');expect(home).toContain('href="#pdf-tools"');expect(home).toContain('{PDF_TOOLS.length} tools available');expect(home).toContain('href="/calculators"');expect(home).toContain('{AVAILABLE_CALCULATORS.length} tools available');expect(home).toContain('href="/converters"');expect(home).toContain('{AVAILABLE_CONVERTERS.length} tools available');expect(AVAILABLE_CALCULATORS.length).toBeGreaterThan(20);expect(AVAILABLE_CONVERTERS.length).toBeGreaterThan(15);for(const label of ['Documents','Text'])expect(home).toContain(`title:'${label}'`);expect(home).toContain('aria-disabled="true"');expect(home).toContain('Coming soon');expect(home).not.toMatch(/href=["'{`]\/(documents?|text)/i)})
