...
describe('expanded Tooliyapa catalog',()=>{
  it('publishes all registry routes as actual pages',()=>{expect(AVAILABLE_CALCULATORS).toHaveLength(26);expect(AVAILABLE_CONVERTERS).toHaveLength(18);expect(AVAILABLE_DOCUMENT_TOOLS).toHaveLength(14);expect(AVAILABLE_TEXT_TOOLS).toHaveLength(15);for(const tool of [...AVAILABLE_CALCULATORS,...AVAILABLE_CONVERTERS,...AVAILABLE_DOCUMENT_TOOLS,...AVAILABLE_TEXT_TOOLS]){expect(PUBLIC_ROUTES).toContain(tool.href);expect(fs.existsSync(`app${tool.href}/page.js`)).toBe(true)}})
  it('keeps every shared page backed by substantive content',()=>{expect(Object.keys(MORE_CALCULATOR_PAGES)).toHaveLength(15);expect(Object.keys(CONVERTER_PAGES)).toHaveLength(18);expect(Object.keys(DOCUMENT_PAGES)).toHaveLength(14);expect(Object.keys(TEXT_PAGES)).toHaveLength(15);for(const page of Object.values(MORE_CALCULATOR_PAGES))expect(page.sections.length).toBeGreaterThanOrEqual(3);for(const page of Object.values(CONVERTER_PAGES))expect(page.sections.length).toBeGreaterThanOrEqual(3);for(const page of [...Object.values(DOCUMENT_PAGES),...Object.values(TEXT_PAGES)]){expect(page.about.length).toBeGreaterThan(80);expect(page.limitations.length).toBeGreaterThan(60)}})
