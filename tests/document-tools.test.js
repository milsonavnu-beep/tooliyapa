...
describe('document tools', () => {
  ...
  it('blocks unsafe archive paths', () => {
    ...
    for (const path of ['../secret.txt', '/absolute.txt', 'C:/unsafe.txt', 'word/../secret.xml']) {
      expect(() => normalizeArchivePath(path)).toThrow(/unsafe path/i)
    }
  })

  it('creates and reads a bounded ZIP package without a server dependency', async () => {
    const zip = await createZip([
      { name: 'word/document.xml', data: '<document>Hello</document>' },
      { name: 'docProps/core.xml', data: '<title>Sample</title>' },
    ])
