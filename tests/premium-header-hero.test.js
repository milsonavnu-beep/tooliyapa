...
describe('premium header and homepage hero',()=>{
  ...
  it('renders one broad homepage heading and keeps substantive content',()=>{expect(home.match(/<h1(?:\s|>)/g)).toHaveLength(1);expect(home).toContain('Everyday tools for');expect(home).toContain('documents, calculations,');for(const heading of ['What you can do with Tooliyapa','How browser-based processing works','Choose the right PDF tool','Practical tools, clear expectations'])expect(home).toContain(heading)})
  it('keeps PDF registry routes and expands search to active categories',()=>{expect(PDF_TOOLS).toHaveLength(10);for(const tool of PDF_TOOLS)expect(PUBLIC_ROUTES).toContain(tool.href);expect(search).toContain("import { PDF_TOOLS } from '@/lib/tools'");expect(search).toContain("import { AVAILABLE_CONVERTERS } from '@/lib/converters'");expect(header).toContain("import { PDF_TOOLS } from '@/lib/tools'")})
  it('activates calculators and converters while keeping Documents and Text future',()=>{for(const label of ['Documents','Text'])expect(header).toContain(`'${label}'`);expect(header).toContain('href="/calculators"');expect(header).toContain('href="/converters"');expect(header).not.toMatch(/href=["'{`]\/(documents?|text)/i);expect(PUBLIC_ROUTES).toContain('/calculators');expect(PUBLIC_ROUTES).toContain('/converters');expect(PUBLIC_ROUTES).not.toEqual(expect.arrayContaining(['/documents','/text']))})
